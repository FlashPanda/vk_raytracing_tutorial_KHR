...
void HelloVulkan::initRayTracing() {
    ...
    prop2.pNext = &m_rtProperties;  // But it is a void, right? How can I determine what type of pointer can be used?
...
auto HelloVulkan::objectToVkGeometryKHR(const ObjModel& model) {
    // BLAS builder requires raw devcie addresses
    ...
    // Identify the above data as containing opaque trangles.
...
// The resulting acceleration structures will be stored within the helper in the order of construction,
// so that they can be directly referenced by index later.
void HelloVulkan::createBottomLevelAS()
{
    // BLAS - Stroing each primitive in a geometry
...
void HelloVulkan::createTopLevelAS() {
    ...
    for (const HelloVulkan::ObjInstance& inst : m_instances)
    {
        ...
        rayInst.mask                           = 0xFF;  // Only be hit if rayMas & instance.mask != 0
...
void HelloVulkan::createRtPipeline()
{
  ...
  // The pipeline uses two descriptor sets: set=0 is specific to the ray tracing pipeline (TLAS and output image), and set=1 is shared with the rasterization (scene data):
  ...
  // A ray generation or miss shader is a group by itself, but hit groups can comprise up to 3 shaders(intersection, any hit , closest hit)
...
void HelloVulkan::createRtShaderBindingTable() {
  ...
  // We fetch the handles to the shader groups of the pipeline.
  ...
  m_debug.setObjectName(m_rtSBTBuffer, std::string("SBT")); // Give it a debug name for NSight
  ...
  // Helper to retrive the handle data
...
void HelloVulkan::raytrace(const VkCommandBuffer& cmdBuf, const glm::vec4& clearColor) {
  ...
  m_pcRay.lightPosition = m_pcRay.lightPosition;
  ...
  vkCmdTraceRaysIndirectKHR(cmdBuf, &m_rgenRegion, &m_missRegion, &m_hitRegion, &m_callRegion, m_size.width, m_size.height, 1);
  m_debug.endLabel(cmdBuf);
}